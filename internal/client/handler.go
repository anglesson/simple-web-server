...
func (ch *ClientHandler) UpdateView(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		// Redirecionar para a lista de clientes com mensagem de erro
	...
	// Verificar se há dados de formulário em cookies (após erro de validação)
	if formCookie, err := r.Cookie("form"); err == nil {
		if formData, err := url.QueryUnescape(formCookie.Value); err == nil {
			var form map[string]interface{}
			if json.Unmarshal([]byte(formData), &form) == nil {
				templateData["Form"] = form
			}
		}
	}

	// Verificar se há erros em cookies
	if errorsCookie, err := r.Cookie("errors"); err == nil {
		if errorsData, err := url.QueryUnescape(errorsCookie.Value); err == nil {
			var errors map[string]string
			if json.Unmarshal([]byte(errorsData), &errors) == nil {
				templateData["Errors"] = errors
			}
		}
	}

	ch.templateRenderer.View(w, r, "client/update", templateData, "admin")
}
